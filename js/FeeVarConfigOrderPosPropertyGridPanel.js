...
Ext.ux.wam.PropertyColumnModel = function(store){
    ...
    Ext.ux.wam.PropertyColumnModel.superclass.constructor.call(this, [
        {header: Ext.grid.PropertyColumnModel.prototype.nameText, width:100, sortable: true, dataIndex:'name', id: 'name'},
        {header: Ext.grid.PropertyColumnModel.prototype.valueText, width:200, resizable:false, dataIndex: 'value', id: 'value'},
...
Tine.Membership.FeeVarConfigOrderPosPropertyGridPanel = Ext.extend(Ext.ux.wam.PropertyGrid, {
	 layout: 'border',
	 mode:'remote',
	 border: false,
	 recordClass: null,
	 ...
	 initComponent: function(){
		 ...
		 this.initRecord();
		 Tine.Membership.FeeVarConfigOrderPosPropertyGridPanel.superclass.initComponent.call(this);
		 this.on('afteredit', this.onEditProperty, this);
	 },
	 initStore: function(){
	 ...
	 setOrderPostionRecord: function(orderPositionRecord){
		 this.orderPositionRecord = orderPositionRecord;
		 this.record.set('id', this.orderPositionRecord.get('id'));
		 this.record.id = this.orderPositionRecord.get('id');
		 this.initRecord();
		 // save initially
		 //this.loadFeeVarConfigOrderPos();
	 },
	 loadFeeVarConfigOrderPos: function(){
		 var orderPosId = this.orderPositionRecord.get('id');
		 Ext.Ajax.request({
            scope: this,
            success: this.onLoadVarConfigOrderPos,
            params: {
                method: 'Membership.getVarConfOrderPosByOrderPosId',
                orderPosId:  orderPosId
            },
		    failure: function(){
		    	//console.log('no order pos - ok');
		    }
		 });
	 },
	 onLoadVarConfigOrderPos: function(response){
		 var result = Ext.util.JSON.decode(response.responseText);
		 this.record = new Tine.Membership.Model.FeeVarOrderPos(result);
		 this.onRecordLoad();
	 },
	 getCustomEditor: function(editor){
		 return new Ext.grid.GridEditor(editor);
	 },
	 getDataFromStore: function(data){
		 this.store.each( function(item){
			this[item.data.name] = item.data.value; 
		},data);
	 },
	 getDataFromRecord: function(){
		 var use = this.record.get('use_fee_var_config_id');
		 var amount = this.record.get('amount_fee_var_config_id');
		 var priceNetto = this.record.get('price_netto_fee_var_config_id');
		 var priceBrutto = this.record.get('price_brutto_fee_var_config_id');
		 var name = this.record.get('name_fee_var_config_id');
		 var factor = this.record.get('factor_fee_var_config_id');
			...
			var data = {
					properties: [{
						...
						'text': 'Benutzen',
						'value': use,
						'editor': this.getCustomEditor(Tine.Membership.Custom.getFeeVarConfigRecordPicker(
								'use_fee_var_config_id',
								{
									width:200
								}
						).setFeeDefinition(this.feeDefinitionRecord)),
						renderer: Tine.Membership.renderer.feeVarConfigRenderer
					},{
						'name': 'amount_fee_var_config_id',
						'text': 'Menge',
						'value': amount,
						'editor': this.getCustomEditor(Tine.Membership.Custom.getFeeVarConfigRecordPicker(
								'amount_fee_var_config_id',
								{
									width:200
								}
						).setFeeDefinition(this.feeDefinitionRecord)),
						...
						'value': priceBrutto,
						'editor': this.getCustomEditor(Tine.Membership.Custom.getFeeVarConfigRecordPicker(
								'price_brutto_fee_var_config_id',
								{
									width:200
								}
						).setFeeDefinition(this.feeDefinitionRecord)),
    ...
    onRecordLoad: function() {
        ...
        if (! this.record.id) {
            this.setTitle(String.format(_('Add New {0}'), this.i18nRecordName));
        } else {
            this.setTitle(String.format(_('Edit {0} "{1}"'), this.i18nRecordName, this.record.getTitle()));
        }
        
        if (this.fireEvent('load', this) !== false) {
            
        	this.store.loadData(this.getDataFromRecord());
            this.loadMask.hide();
        }
    },
    ...
    onRecordUpdate: function() {
       // get record from properties
    	this.getDataFromStore(this.record.data);
    	
    	if(this.orderPositionRecord){
    		this.record.set('order_pos_id', this.orderPositionRecord.get('id'));
    	}
    },
